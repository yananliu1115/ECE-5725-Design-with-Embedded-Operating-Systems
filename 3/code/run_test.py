...
while time.time()-code_run<100 and flag:
	...
	if counter%2==0:
		...
		if process == 1:
			wheel_control(1,2)
			wheel_control(2,2)
			left_log.appendleft(("FRD",int(time.time()-time_start)))
			left_log.pop()
			right_log.appendleft(("FRD",int(time.time()-time_start)))
			right_log.pop()
			time.sleep(3)
			process = 2
		elif process == 2:
			...
			wheel_control(1,1)
			wheel_control(2,1)
			left_log.appendleft(("BRD",int(time.time()-time_start)))
			left_log.pop()
			right_log.appendleft(("BRD",int(time.time()-time_start)))
			right_log.pop()
			time.sleep(3)
			process = 4
		elif process == 4:#left
